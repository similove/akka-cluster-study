...
import com.zjw.entity.{RecallRequest, ServerRegistration}
import com.zjw.rpc.akka.ZookeeperClusterSeed
import com.zjw.util.JacksonUtil
import org.apache.logging.log4j.scala.Logging

/**
 * @author zjw <cn.zjwblog@gmail.com> 2019/12/17 上午10:13
 *
 */
class RecallServer extends Actor with Logging {
  ...
  def register(member: Member): Unit = {
    logger.info(s"send recall register message to ${member.address} with role ${member.roles}.")
    if (member.hasRole("engine")) {
      context.actorSelection(RootActorPath(member.address) / "user" / "engine") !
        ServerRegistration("recall")
...
object RecallServer extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    RecallServer.initAkkaCompone()
    AkkaContext.getRouter("recall", classOf[RecallServer], "my-dispatcher")
    var cnt = 1
    while (true) {
      Thread.sleep(60000)
